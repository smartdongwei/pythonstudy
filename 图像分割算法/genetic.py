...
class genetic:
    ...
    def get_fitness(self):
        ...
        :return: 返回方差列表
        """
        fitness = [fast_ostu(self.image, i) for i in self.population]  #求得所有像素点对应的前景像素和背景像素的方差列表
        return fitness
    ...
    def get_threshold(self):
        flag = 0
        fitness = self.get_fitness()   #得到所有像素点的方差列表
        best_fitness = np.max(fitness)   #当方差最大时，对应的像素值为图像的全局阈值
        best_threshold = self.population[np.argmax(fitness)]  #np.argmax 返回最大值的索引
        while True:
